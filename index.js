...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.yqnfvh6.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const usersCollection = client.db("musical-oasis").collection("users");
    const populerClassCollection = client.db("musical-oasis").collection("popular");
    const instructorClassCollection = client.db("musical-oasis").collection("instructor");
    const cartsClassCollection = client.db("musical-oasis").collection("carts");
    const paymentCollection = client.db("musical-oasis").collection("payments");
...
app.listen(port,() =>{
    console.log(`Musical oasis is running on port${port}`)
})